...
namespace Types
{
    ...
    public:
        ...
        inline Type& Insert(SizeType index, const Type& element)
        {
        ...
        [[nodiscard]] inline SizeType IndexOf(const Type& element) const
        {
            const Type* base = Dense.data();
            const Type* pointer = &element;

            if (pointer < base || pointer >= base + Dense.size())
            {
                // TODO: throw
            }

            SizeType denseIndex = static_cast<SizeType>(pointer - base);

            return ReverseMapping[denseIndex];
        }
        ...
        [[nodiscard]] inline std::vector<SizeType>& GetSparse()
        {
            return Sparse;
        }

        [[nodiscard]] inline const std::vector<SizeType>& GetSparse() const
        {
            return Sparse;
        }
